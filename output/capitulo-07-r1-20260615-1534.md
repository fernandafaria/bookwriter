...
O erro dele é o mesmo de 90% dos times que implementam IA: eles mantêm os mesmos rituais de antes e esperam resultados diferentes.

**Operating System** de um time de produto com IA não é sobre tecnologia. É sobre rituais de decisão. É sobre o conjunto de práticas que define como um time opera no dia a dia: como observa, como agenda, como escala, como revisa, como itera.
...
**Conexão com Product Compass (Capítulo 6):** Lembra do Product Compass? A etapa "Validar" do compasso exige exatamente o ritual de Review (O4) que descrevemos aqui. Sem ele, você valida o modelo, não a experiência. O Compass mapeia oportunidades; o OS garante que elas não se transformem em pesadelos de governança.

O AI Product Maturity Model (Gartner, 2024) define 5 níveis de maturidade: Awareness, Active, Operational, Systemic, Transformational. Times nos níveis 1-2 não deveriam introduzir agentes autônomos. Só times nos níveis 3-5 têm a governança necessária para não criar o cenário do Banco Digital: modelo perfeito, experiência horrível.
...
**Exemplo real:** O time do Cursor (IDE com IA) revisa semanalmente sugestões de código rejeitadas pelos desenvolvedores. Cada rejeição é um dado. Cada padrão de rejeição é uma oportunidade de melhoria. Eles não esperam o usuário reclamar. Eles monitoram o comportamento do usuário em relação ao agente.
...
**Exemplo Cursor:** Quando um desenvolvedor rejeita uma sugestão de código, o Cursor não só registra a rejeição. Ele analisa o contexto: o desenvolvedor aceitou sugestão similar antes? O código rejeitado era semanticamente incorreto ou só diferente do estilo preferido? Cada rejeição vira dado de treinamento para o próximo ciclo. Em 90 dias, a taxa de aceitação de sugestões subiu de 34% para 61%, e o tempo médio de codificação caiu 22%. (Fonte: Cursor Blog, 2025)
